...
public abstract class NyxBlockSlab extends Block implements IFireproofBlock {
    ...
    public final Supplier<NyxBlockSlab> singleSlab;
    public final Supplier<NyxBlockSlab> doubleSlab;

    public NyxBlockSlab(Material material, Supplier<NyxBlockSlab> singleSlab, Supplier<NyxBlockSlab> doubleSlab) {
        super(material);
    ...
    public static NyxBlockSlab[] makeSlab(String baseName, Material material, SoundType soundType, float hardness, float resistance) {
        ...
        NyxBlocks.initBlock(doubl.getValue(), baseName + "_double", null);
        return new NyxBlockSlab[]{singl.getValue(), doubl.getValue()};
    }
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        if (this.isDouble()) return FULL_BLOCK_AABB;
        else return state.getValue(HALF) == EnumBlockHalf.TOP ? AABB_TOP_HALF : AABB_BOTTOM_HALF;
    }
    ...
    public BlockFaceShape getBlockFaceShape(IBlockAccess world, IBlockState state, BlockPos pos, EnumFacing face) {
        if (this.isDouble()) return BlockFaceShape.SOLID;
        else if (face == EnumFacing.UP && state.getValue(HALF) == EnumBlockHalf.TOP) return BlockFaceShape.SOLID;
        else
    ...
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        if (this.isDouble()) return this.getDefaultState();
            ...
            return facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double) hitY <= 0.5D) ? state.withProperty(HALF, EnumBlockHalf.BOTTOM) : state.withProperty(HALF, EnumBlockHalf.TOP);
        }
    }
